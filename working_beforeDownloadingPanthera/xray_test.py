    ...
    try:
        ...
        for hybrid in root.findall(".//Hybrid"):
            ...
            for rd53b in hybrid.findall(rd53_tag):
                ...
                if old_config:
                    # Update the txt file name -should contain module name
                    new_config = old_config.replace(old_config.split("_")[2], module_name)
...
def main():
    ...
    # Define the pattern for tuned TXT files
    tuned_txt_pattern = os.path.join(BASE_DIR, "xray_files/tuned_txt_files", module_name, f"CMSIT_RD53_{module_name}_*.txt")
